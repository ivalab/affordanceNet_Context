...
obj2attrDict = {
  'bowl':     [0,0,0,1,0,0,0],
  'hammer':   [1,0,0,0,1,0,0],
  'knife':    [1,1,0,0,0,0,0],
  'mallet':   [1,0,0,0,1,0,0],
  'mug':      [1,0,0,1,0,0,1],
  'scissors': [1,1,0,0,0,0,0],
  'spoon':    [1,0,1,0,0,0,0],
  ...
  'saw':       [1,1,0,0,0,0,0],
  'scoop':     [1,0,1,0,0,0,0],
  'shears':    [1,1,0,0,0,0,0],
  'shovel':    [1,0,0,0,0,1,0],
  'tenderizer':[1,0,0,0,1s,0,0],
  'trowel':    [1,0,1,0,0,0,0],
}
attrInd = [1,1,1,1,1,1,1]
attrIndzeros = [0,0,0,0,0,0,0]
...
#             ################### Toan mask target##########################
            ...
            elif 10254 <= im_ind and im_ind < 11395:
                attr = obj2attrDict['mallet']
            elif 11395 <= im_ind and im_ind < 16645:
                attr = obj2attrDict['mug']
            elif 18025 <= im_ind and im_ind < 20119:
                attr = obj2attrDict['scissors']
            elif 21755 <= im_ind and im_ind < 24624:
                attr = obj2attrDict['spoon']
            elif 26526 <= im_ind and im_ind < 28843:
                attr = obj2attrDict['turner']
            elif 2639 <= im_ind and im_ind < 4194:  # below added for imagewise split (no need to be deleted, works for both splits)
                attr = obj2attrDict['cup']
            elif 8758 <= im_ind and im_ind < 10254:  
                attr = obj2attrDict['ladle']
            elif 16645 <= im_ind and im_ind < 17130:  
                attr = obj2attrDict['pot']
            elif 17130 <= im_ind and im_ind < 18025:  
                attr = obj2attrDict['saw']
            elif 20119 <= im_ind and im_ind < 20714:  
                attr = obj2attrDict['scoop']
            elif 20714 <= im_ind and im_ind < 21327:  
                attr = obj2attrDict['shears']
            elif 21327 <= im_ind and im_ind < 21755:  
                attr = obj2attrDict['shovel']
            elif 24624 <= im_ind and im_ind < 25262:  
                attr = obj2attrDict['tenderizer']
            elif 25262 <= im_ind and im_ind < 26526:  
                attr = obj2attrDict['trowel']
            else:
                print('attribute not recognized ..')
    
            attrArray = np.zeros((labels.shape[0], 7)) # get the mini-batch size
            attrArrayPos = np.zeros((labels.shape[0], 7)) 
            attrArrayShift = np.ones((labels.shape[0], 7))*(-100) # adding large negative term so after sigmoid it maps to zero
