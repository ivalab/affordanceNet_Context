...
from utils.handy_CONTAINABLE_STATE_MULTI import write_pddl
...
good_range = 0.005
loadstate = False
sim_dir = 'tmp_data/agnostic_objdetection'
...
def draw_reg_text(img, obj_info):
    ...
    # put text
    txt_obj = OBJ_CLASSES[obj_id] + ' ' + str(cfd)[:6]
...
def visualize_mask_asus(im, im_ori, rois_final, rois_class_score, rois_class_ind, masks, thresh):
    list_bboxes = []
    list_masks = []
...
def get_list_centroid_and_class():
    fName = os.path.join(sim_dir, 'list_class_center.txt')
...
def run_affordance_net_asus(net, im):
    ...
    return visualize_mask_asus(im, im_ori, rois_final, rois_class_score, rois_class_ind, masks, thresh=CONF_THRESHOLD)
...
def get_coords_from_image_index(width, height):
    ...
    #if len(coords_3D_full.shape) == 1 or (coords_3D_full.shape[0] == 4 and coords_3D_full.shape[1] == 0):
    #    coords_3D_max = np.asarray([np.mean(coords_3D_full[0]), np.mean(coords_3D_full[1]), np.mean(coords_3D_full[2])])
    #else:
...
if __name__ == '__main__':
    ...
    if args.sim and os.path.exists(os.path.join(sim_dir, 'rgb.npy')) and os.path.exists(os.path.join(sim_dir, 'depth.npy')):
        arr_rgb = np.load(os.path.join(sim_dir, 'rgb.npy'))
        arr_depth = np.load(os.path.join(sim_dir, 'depth.npy'))
    ...
    if (arr_rgb.shape[0] > 100 and arr_rgb.shape[1] > 100):
        ...
        subprocess.call('~/fast-downward/fast-downward.py ../pddl/domain_CONTAINABLE.pddl ../pddl/auto_problem_CONTAINABLE_STATE_MULTI.pddl --search "astar(blind())"', shell=True)
    ...
    for action in plan:
        ...
        for arg, obj in enumerate(action[1:]):
            ...
            #     # Add the pose attribute to the action message (+0.1, +0.28, -0.12 are used to transform relative to handy)
                ...
                #obj_pose_3D.position.z = round(coords_3D_max[2], 2) - 0.045
                obj_pose_3D.position.z = 0.1 - 0.045 # to avoid collision
