...
from utils.handy_CONTAINABLE_binary import write_pddl
...
OBJ_CLASSES = ('__background__', 'objectness', 'cup', 'hammer', 'knife', 'ladle', 'mallet', 'mug', 'pot', 'saw', 'scissors','scoop','shears','shovel','spoon','tenderizer','trowel','turner')
...
def rosRGBDCallBack(rgb_data, depth_data):
    ...
    arr_rgb = bridge.imgmsg_to_cv2(rgb_data, "bgr8")
    arr_depth = bridge.imgmsg_to_cv2(depth_data, "32FC1")
    cv_depthimage2 = np.array(arr_depth, dtype=np.float32)

    cv2.imshow('RGB image', arr_rgb)
    cv2.imshow('depth image', cv_depthimage2)
...
def depth_callback(depth):
    ...
    cv_depthimage = bridge.imgmsg_to_cv2(depth, "32FC1")
    cv_depthimage2 = np.array(cv_depthimage, dtype=np.float32)
    print cv_depthimage2
    cv2.imshow('depth image', cv_depthimage2)
    cv2.waitKey(3)
    #return msg
...
if __name__ == '__main__':
    ...
    if args.sim and os.path.exists('tmp_data/agnostic_objdetection/rgb.npy') and os.path.exists('tmp_data/agnostic_objdetection/depth.npy'):
        ...
        #rospy.Subscriber("/camera/rgb/image_color", Image, image_callback, queue_size=2)
        #rospy.Subscriber("/camera/depth_registered/image_raw", Image, depth_callback, queue_size=2)
    ...
    if (arr_rgb.shape[0] > 100 and arr_rgb.shape[1] > 100):
        ...
        subprocess.call('~/fast-downward/fast-downward.py ../pddl/domain_CONTAINABLE.pddl ../pddl/auto_problem_CONTAINABLE_binary.pddl --search "astar(blind())"', shell=True)
    ...
    for action in plan:
        ...
        for arg, obj in enumerate(action[1:]):
            ...
            if len(width) != 0 and len(height) != 0:
                ...
                obj_pose_3D = Pose()
                obj_pose_3D.position.x = round(coords_3D[0], 2) + 0.30
                obj_pose_3D.position.y = round(coords_3D[1], 2) + 0.32
                obj_pose_3D.position.z = round(coords_3D[2], 2) - 0.0178 - 0.045
